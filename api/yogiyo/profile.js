...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!connection || connection.platform !== 'yogiyo') {
    ...
    // 요기요 대시보드로 이동
    await safeNavigate(page, 'https://www.yogiyo.co.kr/mobile/#/owner/dashboard', {
    ...
    const profileInfo = await page.evaluate(() => {
      return {
        store_name: document.querySelector('.restaurant-name, h1, [data-restaurant-name]')?.textContent?.trim() || '',
        ...
        profile_image: document.querySelector('.restaurant-image img, .profile-image img')?.src || null,
